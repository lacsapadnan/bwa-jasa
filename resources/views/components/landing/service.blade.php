    ...
    <div class="w-96 h-auto overflow-hidden md:p-5 p-4 bg-white rounded-2xl inline-block">
        <div class="flex items-center space-x-2 mb-6">
            ...
            @if ($item->user->detail_user->photo != NULL)
                <img class="w-14 h-14 object-cover object-center rounded-full mr-1" src="{{ url(Storage::url($item->user->detail_user->photo)) }}" alt="photo freelancer" loading="lazy">
            @else
        ...
        @if ($item->thumbnail_service[0]->thumbnail != NULL)
            <img class="rounded-2xl w-full h-48 object-cover" src="{{ url(Storage::url($item->thumbnail_service[0]->thumbnail)) }}" alt="photo freelancer" loading="lazy">
        @else
