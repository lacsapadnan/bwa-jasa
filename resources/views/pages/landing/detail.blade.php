                        ...
                        <div class="flex overflow-x-scroll hide-scroll-bar dragscroll">
                            <div class="flex mt-2 flex-nowrap">
                                @forelse ($thumbnail as $item)
                                    <img class="inline-block mr-2 rounded-lg cursor-pointer h-20 w-36 object-cover" :class="{'border-4 border-serve-button': active == {{ $item->id }}}" src="{{ url(Storage::url($item->thumbnail)) }}" alt="thumbnail service" @click="changeThumbnail('{{ url(Storage::url($item->thumbnail)) }}', {{ $item->id }})">
                                @empty
                                    {{-- Empty --}}
                                @endforelse
                            </div>
                                    ...
                                    <div class="flex items-center col-span-12 p-2 lg:col-span-6">
                                        <div class="flex items-center space-x-4">
                                            @if ($service->user->detail_user->photo != NULL)
                                                <img src="{{ url(Storage::url($service->user->detail_user->photo)) }}" alt="Photo profile" class="w-20 h-20 rounded-full " loading="lazy">
                                            @else
                    ...
                    <div class="mb-4 border rounded-lg border-serv-testimonial-border">
                        ...
                        <div class="flex items-center px-2 py-3 mx-4 mt-4 border rounded-full border-serv-testimonial-border">
                            <div class="flex-1 text-sm font-medium text-center">
                                ...
                                {{ $service->delivery_time ?? '' }} Days Devlivery
                            </div>
                            <div class="flex-1 text-sm font-medium text-center">
                                <svg class="inline" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
