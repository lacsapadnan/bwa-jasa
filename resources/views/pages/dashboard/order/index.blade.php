                                ...
                                @forelse ($order as $key => $order )
                                    <tr class="text-gray-700 border-b">
                                            ...
                                            <div class="flex items-center text-sm">
                                                <div class="relative w-10 h-10 mr-3 rounded-full md:block">
                                                     @if ($order->user_buyer->detail_user->photo != NULL)
                                                        <img class="object-cover w-full h-full rounded-full" src="{{ url(Storage::url($order->user_buyer->detail_user->photo)) }}" alt="buyer photo profile" loading="lazy" />
                                                    @else
                                                ...
                                                <div>
                                                   <p class="font-medium text-black">{{ $order->user_buyer->name ?? '' }}</p>
                                                    <p class="text-sm text-gray-400">{{ $order->user_buyer->detail_user->job ?? '' }}</p>
                                                </div>
                                            </div>
                                        </td>
                                        ...
                                        @if ($order->order_status_id == 1)
                                            <td class="px-1 py-5 text-xs text-red-500"></td>
                                        @else
                                        ...
                                        <td class="px-1 py-5 text-sm">
                                            ...
                                            @elseif ($order->order_status_id == 4)
                                                <a href="{{ route('member.accept.order', $order->id) }}" class="px-4 py-2 mt-1 mr-2 text-center text-white rounded-xl bg-serv-button">Accept</a>

                                                <a href="{{ route('member.reject.order', $order->id) }}" class="px-4 py-2 mt-1 mr-2 text-center bg-white rounded-xl">Reject</a>
                                            @else()
                                                {{ 'N/A' }}
                                            @endif
                                        </td>
