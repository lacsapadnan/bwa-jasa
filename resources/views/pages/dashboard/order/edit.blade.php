                            ...
                            <tbody class="bg-white">
                                <tr class="mb-10 text-gray-700">
                                        ...
                                        <div class="flex items-center text-sm">
                                            <div class="relative w-10 h-10 mr-3 rounded-full md:block">
                                                @if ($order->user_freelancer->detail_user->photo != NULL)
                                                    <img class="object-cover w-full h-full rounded-full" src="{{ url(Storage::url($order->user_freelancer->detail_user->photo)) }}" alt="freelancer photo profile" loading="lazy" />
                                                @else
                                                    <svg class="inline w-12 h-12 mr-3 rounded-full text-gray-300" fill="currentColor" viewBox="0 0 24 24">
                                                        <path d="M24 20.993V24H0v-2.9966A14.977 14.977 0 0112.004 15c4.984 0 9.26 2.354 11.996 5.993zM16.002 8.999a4 4 0 11-8 0 4 4 0 018 0z" />
                                                    </svg>
                                                @endif
                                        ...
                                        <div class="flex items-center text-sm">
                                            <div class="relative w-10 h-10 mr-3 rounded-full md:block">
                                                @if ($order->service->thumbnail_service[0]->thumbnail != NULL)
                                                    <img class="object-cover w-full h-full rounded" src="{{ url(Storage::url($order->service->thumbnail_service[0]->thumbnail)) }}" alt="order thumbnail" loading="lazy" />
                                                @else
                                                    <svg class="inline w-12 h-12 mr-3 rounded-full text-gray-300" fill="currentColor" viewBox="0 0 24 24">
                                                        <path d="M24 20.993V24H0v-2.9966A14.977 14.977 0 0112.004 15c4.984 0 9.26 2.354 11.996 5.993zM16.002 8.999a4 4 0 11-8 0 4 4 0 018 0z" />
                                                    </svg>
                                    ...
                                    <td class="px-1 py-5 text-sm">
                                        {{ '$'.number_format($order->service->price) ?? '' }}
                                    </td>
                        ...
                        <form action="{{ route('member.order.update', [$order->id]) }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="flex p-8 border border-gray-200 rounded-lg bg-serv-upload-bg h-128">
                                <div class="m-auto text-center">
                                    <img src="{{ asset('/assetss/images/services-file-icon.svg') }}" alt="" class="w-20 mx-auto">
                                    <h2 class="mt-8 mb-1 text-2xl font-semibold text-gray-700">
                                        @if (isset($order->file))
                                            {{ substr($order->file, -10) ?? '' }}
                                        @else
                                            Please upload project file before submit
                                        @endif
                                    </h2>
                                    <p class="text-sm text-gray-400">
                                ...
                                @if ($errors->has('file'))
                                    <p class="text-red-500 mb-3 text-sm">{{ $error->first('file') }}</p>
                                @endif
                                ...
                                <div class="p-1 mt-5">
                                    <div class="grid grid-cols-6 gap-6">
                                        <div class="col-span-6">
                                            ...
                                            <textarea placeholder="Enter your note here.." type="text" name="note" id="note" autocomplete="note" class="block w-full py-3 mt-1 border-gray-300 rounded-md shadow-sm focus:ring-green-500 focus:border-green-500 sm:text-sm" rows="4" required {{ isset($order->file) ? 'disabled' : '' }}>{{ $order->service->note }}</textarea>

                                            @if ($errors->has('note'))
                                                <p class="text-red-500 mb-3 text-sm">{{ $error->first('note') }}</p>
                                            @endif
